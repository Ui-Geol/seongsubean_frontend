...
async function fetchLoginEmail() {
  try {
    const res = await common.get('/api/reportboards/auth/email');
    if (res.data.success) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    const res = await common.get(`/api/reportboards/detail/${reportId}`);
    const data = res.data;
...
async function deleteHandler() {
  ...
  try {
    const res = await common.delete(`/api/reportboards/${reportId}`);
    if (res.data.deleted) {
      alert("삭제되었습니다.");
